...
def create_app(data_dir: Path | None = None, interpreter=None, public_demo: bool | None = None,
    ...
    async def lifespan(app: FastAPI):
        ...
        app.state.connections = ConnectionService(registry, settings=env)
    ...
    async def request_boundary(request: Request, call_next):
        ...
        try:
            ...
            if response is None and request.method == "POST":
                ...
                if demo_mode and (is_upload or path.endswith("/configure") or path.startswith("/api/v1/connections")):
                    response = JSONResponse(status_code=403, content={"error": "Onboarding is disabled in public demo mode."})
                else:
                    limit = MAX_UPLOAD_BYTES if is_upload else 65536 if path.endswith("/configure") or path.startswith("/api/v1/connections") else MAX_BODY_BYTES
                    body = bytearray()
