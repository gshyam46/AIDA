...
def connections(request: Request):
    return request.app.state.connections.list(principal(request))
...
def create_connection(request: Request, config: dict = Body(...)):
    ...
    if limited is not None:
        return limited
    result = request.app.state.connections.create(config, principal(request))
    user = principal(request)
    if user:
        request.app.state.auth.record_event("connection_created", user["id"], client_fingerprint(request))
...
def connection_action(connection_id: str, action: str, request: Request, config: dict = Body(default={})):
    ...
    if limited is not None:
        return limited
    result = request.app.state.connections.action(connection_id, action, config, principal(request))
    user = principal(request)
    if user:
        request.app.state.auth.record_event("connection_changed", user["id"], client_fingerprint(request))
    return result
