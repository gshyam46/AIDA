...
from backend.core.semantic import SemanticResult
...
class RelationalStub:
    """Explicitly marked API test stub. Real-model proof lives in evaluation."""
    def __init__(self):
        self.calls = []
    ...
    def parse(self, question, catalog, as_of=None):
        ...
        return SemanticResult(copy.deepcopy(chinook_cases()[0]["plan"]), {"api_test_stub": True}, {"model_calls": 1})


def test_relational_api_routes_selected_source_and_rejects_cross_source_plan(tmp_path):
    parser = RelationalStub()
    with TestClient(create_app(tmp_path, semantic_parser=parser)) as client:
        ...
        assert result["semantic_ir"]["api_test_stub"] and parser.calls == [("Units sold by customer country", "chinook")]
...
def test_uploaded_relational_mapping_persists_and_stays_private(tmp_path):
    source = chinook_source()
    with TestClient(create_app(tmp_path, semantic_parser=RelationalStub())) as client:
    ...
    with TestClient(create_app(tmp_path, semantic_parser=RelationalStub(), public_demo=True)) as client:
    ...
    with TestClient(create_app(tmp_path, semantic_parser=RelationalStub(), public_demo=False)) as client:
        result = client.post("/api/v1/query", json={"source_id": source_id, "plan": chinook_cases()[0]["plan"]}).json()
        assert result["success"]
