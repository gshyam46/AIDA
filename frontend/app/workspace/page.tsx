...
const CHART_TYPES = ['bar', 'line', 'area', 'donut', 'scatter', 'stat', 'table']
const REASONS: Record<string, string> = {ambiguous: 'Ambiguous request', unknown_term: 'Name not in the catalog', unsupported: 'Outside supported analysis', sensitive_data: 'Restricted data', prompt_injection: 'Instruction blocked', vague: 'Needs a specific measure'}
...
const validSavedCard = (card: unknown): card is SavedCard => {
  ...
  if (!(typeof item.id === 'string' && typeof item.source_id === 'string' && typeof item.title === 'string' && item.title.length <= 1500 && plan && CHART_TYPES.includes(item.chartType))) return false
  if (plan.calculations !== undefined && (!Array.isArray(plan.calculations) || plan.calculations.length > 3)) return false
...
function QueryTelemetry({result}: {result: QueryResponse}) {
  ...
  return <div className="trust-metrics telemetry">
    ...
    <div><Zap size={16}/><span>Interpretation<strong>{sourceLabel(meta.interpretation_source)}</strong>{meta.pipeline && <small>{meta.pipeline === 'two_stage' ? 'Resolve names, then plan' : 'Single structured call'}{meta.guard_calls ? ' · prompt-attack screen' : ''}</small>}</span></div>
    <div><Activity size={16}/><span>Model calls this request<strong>{meta.model_calls ?? 'Not reported'}</strong>{meta.model_latency_ms ? <small>{Math.round(meta.model_latency_ms)} ms model time</small> : null}</span></div>
    <div><Database size={16}/><span>Model<strong>{meta.model || meta.model_name || (meta.model_calls === 0 ? 'Not invoked' : 'Not reported')}</strong>{meta.estimated_model_cost_usd ? <small>≈ ${meta.estimated_model_cost_usd.toFixed(5)} list price</small> : null}</span></div>
...
function WorkspaceApp() {
  ...
  const signedOut = useCallback((error: unknown) => {
    if (error instanceof ApiError && error.status === 401) {router.replace('/login?next=/workspace'); return true}
    return false
  }, [router])
  useEffect(() => {
    let active = true
    getSession().then(state => {
      if (!active) return
      if (state.auth_required && !state.user) {router.replace('/login?next=/workspace'); return}
      if (state.auth_required && !state.onboarding) {router.replace('/onboarding'); return}
      setSession(state)
    }).catch(() => {if (active) setSession({auth_required: false, signup_enabled: false, user: null, onboarding: null, hosted_inference: false})})
    return () => {active = false}
  }, [router])
  const loadSource = useCallback(async (id: string, list: SourcesResponse) => {
    ...
    } catch (error) {if (sequence === requestSequence.current && !signedOut(error)) setBootError(error instanceof Error ? error.message : 'The data service could not be reached.')}
  ...
  const boot = useCallback(async (preferredId?: string) => {
    ...
    } catch (error) {if (!signedOut(error)) {setBootError(error instanceof Error ? error.message : 'Could not load data sources.'); setBooting(false)}}
  }, [loadSource, signedOut])
  useEffect(() => {
    if (!session) return
    const params = new URLSearchParams(window.location.search)
    if (params.get('tab') === 'data') setTab('data')
    void boot(params.get('source') || undefined)
  ...
  const refreshModel = async () => {try {setSources(await getSources())} catch (error) {if (!signedOut(error)) setToast(error instanceof Error ? error.message : 'Could not check model status.')}}
  const logout = async () => {await signOut().catch(() => undefined); router.replace('/login')}
  const run = async (input: QueryRequest, title?: string) => {
    ...
    try {
      ...
      setResult(response); setResultTab('chart'); setChartType(response.calculations?.length && planDimensions(response.plan).length ? 'bar' : response.chart?.type || 'bar')
      if (response.plan) setBuilder(response.plan)
    } catch (error) {
      if (sequence !== requestSequence.current || signedOut(error)) return
  ...
  const drill = (dimension: string, value: string) => {
    ...
    if (isRelational(result.plan)) {
      ...
      const plan: RelationalQueryPlan = {...result.plan, calculations: undefined, post: undefined, dimensions, filters: [...result.plan.filters.filter(filter => filter.field !== dimension), {field: dimension, op: 'eq', value: filteredValue}], comparison: dimensions.length ? result.plan.comparison : null, sort: {field: result.plan.metrics[0], direction: 'desc'}}
    ...
    const plan = {...result.plan, calculations: undefined, post: undefined, dimension: nextDimension, filters: {...result.plan.filters, [dimension]: value}, sort: nextDimension === 'month' ? 'dimension_asc' as const : 'value_desc' as const}
  ...
  const view = result ? presentationView(result, catalog) : null
  ...
  const hosted = sources?.model?.inference_location === 'hosted'
  ...
  const chartOptions = view && isRelational(view.result.plan) ? relationalChartTypes(view.result, view.catalog) : legacyResultPlan?.dimension ? ['bar', 'line'] : []
  ...
  const formatCell = (key: string, value: string | number | null) => {
    ...
    const calculation = result?.calculations?.find(item => item.id === key)
    if (calculation?.format === 'percent') return new Intl.NumberFormat('en-US', {style: 'percent', maximumFractionDigits: 2}).format(value)
    if (calculation) return formatValue(value, calculation.format === 'currency' ? calculation.inputs[0] : '', false, catalog)
    if (key === 'value' || metricIds.includes(key)) return formatValue(value, key === 'value' ? metricIds[0] || '' : key, false, catalog)
    return String(value)
  }
  if (!session) return <div className="loading-panel" style={{minHeight: '100vh'}} role="status"><LoaderCircle className="spin" size={25}/><strong>Opening your AIDA workspace…</strong></div>
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${sidebarOpen ? 'is-open' : ''}`}>
      ...
      <div className="sidebar-bottom"><div className="private-card"><ShieldCheck size={22}/><strong>Language understands.<br/>Code verifies.</strong><p>{hosted ? 'An LLM resolves your words to approved definitions. Code validates every plan and runs read-only SQL.' : 'A local model interprets questions. Approved definitions and read-only SQL control each answer.'}</p><span><span className="status-dot"/>Database rows never enter model prompts</span></div>
        {session.user ? <div className="user-menu"><div>{session.user.name.slice(0, 2)}</div><span><strong>{session.user.name}</strong><small>{session.onboarding?.company || session.user.email}</small></span><button aria-label="Sign out" title="Sign out" onClick={() => void logout()}><LogOut size={15}/></button></div>
          : <div className="profile"><div>AI</div><span><strong>{sources?.uploads_enabled ? 'Local workspace' : 'Demo workspace'}</strong><small>{hosted ? 'Hosted interpretation' : 'Private inference'}</small></span><LockKeyhole size={14}/></div>}
      </div>
    </aside>
    <div className="main-shell">
      <header className="topbar"><div className="breadcrumbs"><button className="mobile-menu icon-button" aria-label="Open navigation" onClick={() => setSidebarOpen(true)}><PanelLeftClose size={20}/></button><span>Workspace</span><ChevronRight size={13}/><strong>{tab === 'explorer' ? 'Explorer' : tab === 'dashboards' ? 'Dashboards' : 'Data catalog'}</strong></div><div className="topbar-right"><span className="demo-badge"><span/>{activeSource?.synthetic ? 'Synthetic demo' : 'Private source'}</span><span className="topbar-divider"/><button className="text-button engine-status" aria-label="Refresh model status" onClick={() => void refreshModel()}><Activity size={14}/>{modelAvailable === true ? (hosted ? `Model ready · ${sources?.model?.model}` : 'Local model ready') : modelAvailable === false ? 'Model unavailable' : 'Model status unknown'}</button></div></header>
      <main id="main" className="main-content">
        <div className="page-heading"><div><div className="eyebrow">{tab === 'explorer' ? 'YOUR BUSINESS, IN FOCUS' : tab === 'dashboards' ? 'THE BIGGER PICTURE' : 'DEFINE WHAT THE NUMBERS MEAN'}</div><h1>{tab === 'explorer' ? `${session.user ? `${session.user.name.split(' ')[0]}, here's` : 'A clear view of'} your business.` : tab === 'dashboards' ? 'Your dashboard.' : 'Know what you’re querying.'}</h1><p>{tab === 'explorer' ? 'Ask naturally. Inspect the interpretation. Explore the results.' : tab === 'dashboards' ? 'Saved plans run directly against their original data source.' : 'Approve your schema and business definitions before asking questions.'}</p></div><div className="dataset-date"><CalendarDays size={15}/><span>{dateRange(catalog)}</span></div></div>
        ...
        {modelAvailable === false && <div className="connection-error" role="status"><Activity size={19}/><div><strong>The {hosted ? 'hosted' : 'local'} language model is unavailable</strong><p>{sources?.model?.error || 'Natural-language questions need the model service. The visual builder can execute an explicit plan independently.'}</p></div><button className="button secondary" onClick={() => void refreshModel()}><RefreshCw size={14}/>Check model</button></div>}
        {tab === 'explorer' && <>
          ...
          <section className="query-panel panel" aria-labelledby="query-heading"><div className="query-panel-heading"><h2 id="query-heading"><span className="mint-icon"><Search size={17}/></span>What would you like to understand?</h2><span className="quiet-badge"><Zap size={12}/>{hosted ? 'LLM name resolution · validated SQL' : 'Local interpretation · validated SQL'}</span></div>
          ...
          {busy || booting ? <div className="loading-panel panel" role="status"><LoaderCircle className="spin" size={25}/><strong>{booting ? 'Loading your source and catalog…' : interpreting ? 'Interpreting your question…' : 'Executing your validated plan…'}</strong><span>{booting ? 'Opening a catalog-defined preview' : interpreting ? 'The model maps your words to approved definitions. Code validates the plan and compiles read-only SQL.' : 'Direct plan execution · no language interpretation needed'}</span></div> : queryError ? <section className="query-error panel" data-testid="query-error" role="alert"><span className="error-icon"><Search size={22}/></span>{queryError.clarification_reason && <span className="clarify-reason">{REASONS[queryError.clarification_reason] || labelFor(queryError.clarification_reason)}</span>}<h2>{queryError.error_type === 'clarification_required' ? 'Let’s make that a little more specific.' : queryError.error_type === 'misuse_paused' ? 'Questions are paused for now.' : queryError.error_type === 'rate_limited' ? 'Slow down for a moment.' : 'This query couldn’t be completed.'}</h2><p>{queryError.error}</p><div className="error-suggestions">{queryError.suggestions?.slice(0, 4).map(suggestion => <button key={suggestion} className="button secondary" onClick={() => {setMode('question'); void run({question: suggestion})}}>{suggestion}<ArrowRight size={13}/></button>)}</div><div className="error-suggestions">{lastRequest && queryError.error_type !== 'misuse_paused' && <button className="button secondary" onClick={() => void run(lastRequest)}><RefreshCw size={13}/>Retry query</button>}<button className="text-button" onClick={() => setMode('builder')}>Choose exact fields in the visual builder<ArrowRight size={13}/></button></div>{queryError.meta.model_calls !== undefined && <QueryTelemetry result={queryError}/>}</section> : result && view && <div className="analysis-layout"><section className="result-panel panel" data-testid="result-panel" aria-label="Query result"><div className="result-heading"><div><div className="result-eyebrow"><span className="status-dot"/>QUERY COMPLETE<span>{elapsed === undefined ? '' : `${Number(elapsed).toFixed(1)} ms`}</span></div><h2>{resultTitle}</h2><p>{result.plan?.date_from || result.plan?.date_to ? `${result.plan.date_from || 'Beginning'} → ${result.plan.date_to || 'Latest'}` : dateRange(catalog)}<span>·</span>{result.data.length} {result.data.length === 1 ? 'result' : 'results'}{result.calculations?.length ? <><span>·</span>{result.calculations.map(item => item.label).join(', ')}</> : null}</p></div><button className="button secondary save-button" onClick={saveCard} disabled={!hasData}><Plus size={15}/>Save to dashboard</button></div>
            {!!result.interpretation?.notes?.length && <div className="interpretation-notes" aria-label="How your words were interpreted">{result.interpretation.notes.map(note => <span key={note}><Sparkles size={11}/>{note}</span>)}</div>}
            ...
            {resultTab === 'chart' ? <QueryChart key={`${sourceId}-${result.sql}-${JSON.stringify(result.parameters)}-${result.calculations?.length || 0}`} result={view.result} catalog={view.catalog} type={chartType} onSelect={canDrill ? drill : undefined}/> : resultTab === 'table' ? <DataTable key={`${sourceId}-${result.sql}-${result.data.length}`} rows={result.data} columns={result.data.length ? Object.keys(result.data[0]) : []} label={key => columnLabel(key, result, catalog)} format={formatCell} fileName="AIDA-analysis.csv" onExported={count => setToast(`Exported ${count} rows as CSV.`)}/> : <div className="sql-content"><QueryTelemetry result={result}/><QueryLineage result={result}/><p>{result.explanation || metric?.description}</p><label>COMPILED SQL</label><pre>{result.sql}</pre><label>BOUND PARAMETERS</label><pre className="parameters">{JSON.stringify(result.parameters ?? {}, null, 2)}</pre><details><summary>View the validated query plan</summary><pre className="parameters">{JSON.stringify(result.plan, null, 2)}</pre></details>{result.interpretation?.mentions?.length ? <details><summary>View how each phrase was resolved</summary><pre className="parameters">{result.interpretation.mentions.map(item => `“${item.text}” → ${item.label || item.role} (${item.role}, ${item.how})`).join('\n')}</pre></details> : null}<details><summary>View execution metadata</summary><pre className="parameters">{JSON.stringify(result.meta, null, 2)}</pre></details><p className="trust-footnote">{hosted ? 'The hosted model receives your question and approved catalog labels only.' : 'The local model interprets your question using approved metadata.'} Database result rows are never included in model prompts. Code validates every phrase, value and number, then binds filters and executes read-only SQL.</p></div>}
        ...
        {tab === 'data' && <section className="catalog-section"><div className="dataset-banner"><span><Database size={28}/></span><div><div className="eyebrow">CONNECTED DATASET</div><h2>{catalog?.dataset.name || activeSource?.name || 'Data source'}</h2><p>{catalog?.dataset.row_count !== undefined ? `${catalog.dataset.row_count.toLocaleString()} records · ` : ''}{dateRange(catalog)}{catalog?.dataset.currency ? ` · ${catalog.dataset.currency}` : ''}</p></div><span className="demo-badge">{activeSource?.synthetic ? 'Synthetic data' : 'Private source'}</span></div>
          ...
          {catalog && <div className="catalog-grid"><section className="panel catalog-panel"><h2>Measures with explicit definitions</h2><p>Every question and dashboard uses the same approved business logic.</p>{catalog.metrics.map(item => <article className="catalog-metric" key={item.id}><div className="mint-icon"><BarChart3 size={16}/></div><div><h3>{item.label}</h3><p>{item.description}</p><code>{item.id}</code></div></article>)}</section><section className="panel catalog-panel"><h2>Ways to explore</h2><p>Approved dimensions and owner-supplied filter values.</p>{catalog.dimensions.map(item => <article className="catalog-dimension" key={item.id}><h3>{item.label}</h3><div>{item.values?.length ? item.values.map(value => <span key={value}>{value}</span>) : <span>{item.id === 'month' ? 'Calendar month' : 'Exact filters are validated in code'}</span>}</div></article>)}</section></div>}
          <div className="privacy-explainer"><ShieldCheck size={25}/><div><h3>Language understanding with controlled database access.</h3><p>{hosted ? 'Each new question and the approved semantic catalog are sent to the hosted model so it can resolve names and plan the analysis.' : 'The local model receives your question and the approved semantic catalog on this server.'} Result rows, SQL, table names and file paths stay in AIDA. Code checks that every phrase the model cites is in your question, that nothing was dropped or added, and that every value is approved, then compiles read-only SQL. Relational sources use approved table relationships; uploaded databases need an owner-approved catalog and stay private to the account that uploaded them.</p></div></div>
        </section>}
        <footer className="page-footer"><span><span className="mini-brand">AIDA</span>Artificial Intelligence Data Analyst · clarity, without the complexity.</span><span>LLM name resolution · Validated SQL · Source-scoped dashboards</span></footer>
      </main>
    </div>
