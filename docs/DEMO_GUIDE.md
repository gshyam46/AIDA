...
The architecture is **LLM name resolution and planning → code validation → deterministic SQL → calculations in code → interactive result**. It supports accounts, private uploads, rate limits and misuse pauses. It does not claim infallible interpretation, operational database connectors, email verification or multi-factor authentication, horizontal scaling of the in-process rate limits, or a completed public deployment.
